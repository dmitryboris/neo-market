...
@seller_router.get("/me", response_model=SellerResponse, summary="My profile")
async def get_my_data(current_seller: Seller = Depends(get_current_user)):
    return current_seller


@seller_router.patch("/me", response_model=SellerResponse, summary="Update profile")
async def update_me(
...
@seller_router.delete("/me", status_code=status.HTTP_204_NO_CONTENT, summary="Delete account")
async def delete_me(
        session: AsyncSession = Depends(get_session),
        current_seller: Seller = Depends(get_current_user),
):
